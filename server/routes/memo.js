...
router.get('/memo', (req,res)=>{
    Memo.find().sort({"_id":-1})
        .limit(6).exec((err,memos)=>{
            if(err) throw err;
...
router.get('/memo/:listType/:id',(req,res)=>{
...
*/
router.get('/memo/:username',(req,res)=>{
...
*/
router.get('/memo/:listType/:id/:username',(req,res)=>{
    let listType = req.params.listType;
    let id = req.params.id;
...
router.post('/memo/star/:id',(req,res)=>{
    // CHECK MEMO ID VALIDITY
    if(!mongoose.Types.ObjectId.isValid(req.params.id)){
        return res.status(400).json({
            error: 'INVALID ID',
